# EmpiricalAnalysis
Stata 一键实证分析 GUI 工具：傻瓜式操作，一键实证，一键完成常见计量回归，并同步生成可复现的 Stata `.do` 文件。  A GUI toolkit for Stata empirical analysis: simple point-and-click workflow with automatic, reproducible Stata `.do` file generation.
